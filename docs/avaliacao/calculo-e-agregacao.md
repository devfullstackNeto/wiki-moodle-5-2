---
title: Configurar cálculo e agregação
description: Combine notas de forma auditável e coerente com o plano de ensino.
category: Avaliação
audience:
  - Professor
  - Tutor
level: Avançado
readingTime: 6 min
tags:
  - notas
  - cálculo
moodleVersion: 5.2.x
status: em-revisao
lastReviewed: 2026-08-03
---
<ObjectiveCard>Combine notas de forma auditável e coerente com o plano de ensino.</ObjectiveCard>

## Entenda antes de agir

A agregação determina como itens formam o total. Pesos, exclusões, crédito extra e notas vazias mudam o resultado; valide a regra institucional com exemplos numéricos.

<ValidationNotice>Confirme nomes, rotas, capacidades e opções no Moodle institucional antes de publicar instruções passo a passo.</ValidationNotice>

## Decisões essenciais

- Defina a finalidade e quem será afetado.
- Verifique permissões, datas, grupos e dados envolvidos.
- Faça a menor alteração necessária e preserve o histórico.
- Teste com o perfil destinatário e registre o resultado.

<ExampleBox>Em um curso de demonstração, aplique esta orientação com dados fictícios, compare a visão do professor e do estudante e só então replique no curso real.</ExampleBox>

<ScreenshotPlaceholder id="CALCULO-E-AGREGACAO-01" title="Tela de referência" description="Captura mínima necessária para apoiar configurar cálculo e agregação, após validação institucional." alt="Tela do Moodle 5.2 relacionada a configurar cálculo e agregação." src="" />

<Checklist :items='[&quot;Finalidade registrada&quot;, &quot;Impacto sobre estudantes verificado&quot;, &quot;Nomes e opções confirmados no ambiente&quot;, &quot;Teste realizado sem dados pessoais&quot;]' />
